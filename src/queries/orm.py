from sqlalchemy import Integer, and_, func, cast, inspect, or_, select, text, insert
from database import sync_engine, async_engine, session_factory, async_session_factory, Base
from models import ResumesOrm, Workload, WorkersOrm, VacanciesOrm
...
class SyncORM:
    ...
    def update_worker(worker_id: int = 2, new_username: str = "Pepel"):
        with session_factory() as session:
            ...
            # session.expire()
            # session.expire_all()  сбрасывает все изменения и в БД никакой запрос отправлен не будет (синхронная операция)
            session.refresh(worker_bobr) # обновляем состояние до того, которое сейчас в БД
            # т.е. делается запрос и мы получаем новейшее значение worker_bobr
            session.commit()
    ...
    def select_workers_with_joined_relationship():
        with session_factory() as session:
            ...
            query = (
                select(WorkersOrm)
                .options(joinedload(WorkersOrm.resumes))        # for m2o and o2o 
            )
    ...
    def select_workers_with_relationship_contains_eager_with_limit():
        # ознакомиться: https://stackoverflow.com/a/72298903/22259413  
        with session_factory() as session:
    ...
    def select_resumes_with_all_relationships():
        with session_factory() as session:
            ...
            print(f'{result_orm=}')
            result_dto = [ResumesRelVacanciesRepliedWithoutVacancyCompensationDTO.model_validate(
...
class AsyncORM:
    ...
    async def insert_workers():
        ...
        async with async_session_factory() as session:
            ...
            await session.flush() # отправляет в БД все изменения в текущей сессии
    ...
    @staticmethod
    async def update_worker(worker_id: int = 2, new_username: str = 'Bobr'):
        async with async_session_factory() as session:
    ...
    async def insert_resumes():
        async with async_session_factory() as session:
            ...
            resume_jack_1 = ResumesOrm(
                title='Python Junior Developer', compensation = 50000, workload=Workload.fulltime, worker_id=1)
            resumes_jack2 = ResumesOrm(
                title='Python Разработчик', compensation = 150000, workload=Workload.fulltime, worker_id=1)
            resume_michael_1 = ResumesOrm(
                title="Python Data Engineer", compensation=250000, workload=Workload.parttime, worker_id=2)
            resume_michael_2 = ResumesOrm(
                title="Data Scientist", compensation=300000, workload=Workload.fulltime, worker_id=2)
            session.add_all([resume_jack_1, resumes_jack2, resume_michael_1, resume_michael_2])
            await session.commit()
    ...
    async def select_resumes_avg_compensation(like_language: str = 'Python'):
        ...
        async with async_session_factory() as session:
            query = (
                ...
                .select_from(ResumesOrm)
                .filter(and_(
                    ResumesOrm.title.comtains(like_language),
                    ResumesOrm.compensation > 40000
                ))
                .group_by(ResumesOrm.workload)
                .having(func.avg(ResumesOrm.compensation) > 70000)
        ...
        async with async_session_factory() as session:
            ...
            subq = (
                select(
                    r,
                    w,
                    func.avg(r.compensation).over(parttime_by=r.workload).cast(Integer).label('avg_workload_compensation')
                )
                # .select_from(r)
                .join(r, r.worker_id == w.id).subquery('helper1')
            )
